...
**CEVE 543**, "Environmental Data Science", *James Doss-Gollin*.
"""

# ╔═╡ 3c4c3b64-7277-4db4-ab3c-4b2413accdcf
md"""
## Posterior inference (25 points)

Suppose you have a Beta(4, 4) prior distribution on the probability θ that a coin will yield a 'head' when spun in a specified manner.
The coin is independently spun ten times, and 'heads' appear fewer than 3 times.
You are not told how many heads were seen, only that the number is less than 3. 
...
md"""
## Beta Prior (25 Points)

Write a function to perform Bayesian inference about a Poisson distribution, based on count data and using a LogNormal prior distribution.
Specifically, the model is $y \sim \textrm{Poisson}(\theta)$.
...
In this exercise you will use data comes from the NOAA tides and currents dataset for Sewells Point, VA.
The data is sampled hourly for the year 2015 and is available [here](https://tidesandcurrents.noaa.gov/waterlevels.html?id=8638610).
...
md"""
In addition to the `datetime` column, we have `gauge`, which tells us the actual reading on the gauge, `harmonic`, which tells us what would be predicted based on NOAA's complex (and powerful!) sinusoidal method for periodic variability like tides and seasonal cycles, and `weather`, which we define to be the difference between the two (there is also sea level rise, but over the course of one year we can neglect it).

Note that our observations are **in meters**.
"""

# ╔═╡ e7efa1c9-65af-4bbf-a876-8eb63667a957
data = load_data()

# ╔═╡ c9bb37f4-9baf-493f-ae81-5edbb8d2829c
md"Let's take a quick look"
...
md"""
Your task is to analyze one of the three time series available (your choice!)
Model this data using a **Normal IID model**.
"""

# ╔═╡ bc21cd22-2414-4342-a082-7c4d7c332187
md"""
### Maximum likelihood

1. The maximum likelihood estimate for a Normal distribution is a Normal distribution with the sample mean and standard deviation as its parameters. Calculate these and save it as `MLE_sample`
1. Next, explore μ and σ on a 2D grid of possible values. Calculate the log likelihood for each of these values (you may plot if you like) and find the point that has the largest log likelihood. Save this as `MLE_grid`
1. Calculate the maximum likelihood estimator by using the `Optim` package to maximize your log likelihood function. Save this as `MLE_optim`
1. Last, write a generative model in `Turing` and calculate its maximum likelihood. Save this as `MLE_turing`.
1. Do your results match?
"""
...
md"We have already seen how to write a model in Turing for a `1D normal distribution. Modify this if you would like to"
...
# ╔═╡ b8afdf46-c04e-4ebb-8384-c579377b5c79
md"""
## Posterior

1. Modify the `normal1d` model to add some prior information. What priors do you choose and why? (You can edit the function directly, no need to make a new one).
1. Sample 4 chains of 2,500 samples each from the posterior distribution of your model.
1. Use the default `plot` function to plot your posterior. Do any obvious sampling errors jump out?
1. Create a scatter plot of your posterior draws.
"""

# ╔═╡ 496d4620-a693-4d17-b7e4-85b89a4bdae9
md"""
> I chose priors ... because ...
"""

# ╔═╡ d5a382a3-06f2-4b8e-94c9-902c02899b41
md"""
### Model limitations

This is not a perfect model.
Reasoning from the methods you have learned, what assumptions have you made that you think are not valid here?
What might the consequences of breaking these assumptions be?
"""

# ╔═╡ 1d4aeb33-4c70-477a-a33c-6490591ecc54
md"""
> I think...
"""

# ╔═╡ 66f5ce12-47ae-4b44-9596-726b134a5c8e
TableOfContents()
