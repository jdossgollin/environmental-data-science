...
Let's revisit our inference problem in a bit more detail.

- **Given:** Data $y_1, \ldots, y_n$, parametric model $\mathcal{P} = \{p(y|\theta) | \theta ∈ \mathcal{T} \}$.
- **Objective:** Find the distribution in $\mathcal{P}$ which best explains the data. 
- **Reflect:** For what kinds of problem might this be the right objective?

In plain English, this is saying we have to choose a "best" parameter value $\hat\theta$
But what do we mean by "best"?

Maximum Likelihood: the data is best explained by the distribution in $\mathcal{P}$ under which it has the highest likelihood.
```math
\hat\theta_\mathrm{ML} = \arg \max_{\theta \in \mathcal{T}} p(y_1, \ldots, y_n | \theta)
```
...
- the value changes: $\max_y \log g(y) \neq \max_y g(y)$
- the location does not change: $\arg\max_y \log g(y) = \arg\max_y g(y)$

Thus,
```math
\hat\theta_\mathrm{ML} = \arg \max_\theta \prod_{i=1}^n p(x_i | \theta) = \arg \max_\theta \sum_{i=1}^n \log p(x_i | \theta)
```
"""

# ╔═╡ 25542c14-cda3-4591-b3d0-09adce3c7af0
md"""
## Implementing

You can go through and do lots of algebra to derive the maximum likelihood estimator for various distributions.
Many of the problems we'll tackle this week have analytic solutions!
However, I want you to develop tools that scale well to the kinds of problems we'll explore in the coming weeks.

Let's start with our simple coin tossing example, since it's very simple to visualize and wrap our heads around.
Let's also pretend that we don't know about the Binomial distribution, so instead we will model these coin flips as a series of Bernoulli trials (rememeber the link!)
"""

# ╔═╡ ff73c731-8570-471d-8c18-ad8aee215fea
coin_flips = ["H", "H", "H", "T", "H", "H", "H", "H", "H"];

# ╔═╡ 98973390-c154-4dc0-a892-bbd15d9dfdc6
heads = [flip == "H" for flip in coin_flips]

# ╔═╡ 089f6aba-04af-40bc-b9c8-fe8edd534d81
log_likelihood(θ) = sum([logpdf(Bernoulli(θ), x) for x in heads]);

# ╔═╡ c6707b05-764f-46a7-92fa-0a47f7aff1cf
likelihood(θ) = exp(log_likelihood(θ));
...
md"""
We can see by looking at this that the maximum of this likelihood happens around 0.9.
In fact, it doesn't give anything away to tell you that the maximum likelihood estimator for our coin is $y / N = 0.\overline{8}$.
"""
...
md"""
But suppose we didn't know this cool fact.
How could we find this value?
One way is to take the maximum over a grid:
"""
...
We get the same answer if we use the likelihood rather than the log likelihood
"""
...
But again, this might not work so well for more complex problems.
We can get a more precise estimate by using a computer to do the optimization for us!
To do this, we'll use the `Optim.jl` package.
It's pretty complex -- their docs are [here](https://julianlsolvers.github.io/Optim.jl/stable/#user/minimization/) -- but the following syntax works (we need to give either an upper and lower bound, or else a starting point)
"""
...
md"We can extract the maximum likelihood estimate as follows:"

# ╔═╡ b2842ea2-bf1e-48c5-8478-82f5c36716c1
θ̂_ML = res1.minimizer

# ╔═╡ c1701969-0fd6-4e9e-bd92-f31dfb960ec1
md"""
Look, we got a bunch more digits right!
This is a very useful tool.

## Generative MLE with Turing

We can actually make our life a bit simpler by using the powerful `Turing.jl` modeling language.
To use Turing, have a look at [the docs](https://turing.ml/dev/docs/using-turing/get-started).

Turing is great because it lets us separate out different steps of the modeling process.
We'll use it all semester!
The first step is to create a `@model`. This is just a function; its argument should be the data that we pass in.
Within it, we have to define all our parameters.

This is an example of a language that requires us to write down *generative model* for how the data is generated:
"""
...
# ╔═╡ 78686e44-c783-432c-9897-36c1052421b1
md"""
The next step is to combine a model with data.
Here we create a variable where we pass our heads/tails data into our generative model:
"""

# ╔═╡ d95d6683-e925-4fe5-9b92-c8434a8aa165
coinflip_model = coinflip(heads)

# ╔═╡ aeed4daa-7466-41db-8fc7-cc6dd228b671
md"From there, we can easily use `Optim.jl` calculate the maximum likelihood estimate"

# ╔═╡ 89977a80-0808-4028-8f4d-30f4d972ec2d
mle_estimate = optimize(coinflip_model, MLE())

# ╔═╡ 2c03e26b-a4f2-4a9e-b840-ead723287586
mle_estimate.values[1]

# ╔═╡ 0571e09a-fc89-49c0-989c-5db30c5e73b0
md"""
Notice that this is our most accurate answer yet!
Also, try playing around with the prior in the model declaration: you will see that it doesn't matter because the model knows to ignore the prior when maximizing the likelihood.
However, you have to specify *something* so that it knows `p` is a parameter in your model.
"""

# ╔═╡ 3609a6e1-5ca4-41ec-bfcf-5c193ca9ff5e
md"""
## A slightly more complicated problem

Just to get a sense of how this works, let's run through how we would calculate maximum likelihood estimates for a two-parameter probability model.
Specifically, let's say that our data is generated through a Gamma distribution:
"""

# ╔═╡ 74e38fcd-edc1-45bc-aef7-8408bdfa88ef
...
# ╔═╡ f5e6e0fe-a32e-464a-aeb0-3a5128918f06
gamma_data = rand(true_dist, gamma_N) + rand(noise_dist, gamma_N)

# ╔═╡ ed0df1b4-8d6a-4043-b2f5-31152a4647b5
...
# ╔═╡ 978fb51a-1f86-4b21-a675-2198de0aa6ed
gamma_mle = optimize(naive_gamma(gamma_data), MLE())

# ╔═╡ 8b700203-1d52-407f-a81b-4abcdf84bf01
true_dist

# ╔═╡ 43a66334-640f-4657-a69c-a2c6d66c1a30
md"What happens with more data or less noise?"
...
git-tree-sha1 = "d35202933008c1d7cf3e8292bfe7be5a4db41106"
