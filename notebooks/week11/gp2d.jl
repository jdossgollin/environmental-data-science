...
md"""
# 2D Gaussian Processes for Spatial Models

Last week we explored Gaussian Processes as a principled way to model nonlinear patterns in data.
A particularly common application of Gaussian Processes is to the analysis of spatial data, where we can follow the same process:

1. Choose a kernel depending on the relative location of our data (now points will be 2D rather than 1D)
1. Optimize the parameters of the kernel
1. Use the Multivariate Normal to make predictions about new data.
"""

# ╔═╡ 7a2f8f0e-4239-41fc-a0ed-361175355a3c
md"Let's start by generating some true data"

# ╔═╡ 2d62c571-53f3-4866-8901-0a8fb6f142b5
f(x, y) = @. sin(2 * π * x / 10 + 0.9) * cos(2 * π * y / 15.0 + 1.3);

# ╔═╡ 80524a13-c8f5-42d9-8f1e-d6c622663ed1
...
md"""
We could use interpolation, KNN, IDW, or other approaches here!
However, let's get straight to the 2D GPs.
"""

# ╔═╡ d8ddeec5-0003-4ac5-8b0c-93980a40e224
md"""
## Distances

It's often helpful to precompute distances between our points.
In the 1D case we saw that the kernel depended on the distance between points.
We used Euclidean distance but this isn't always the right choice -- for example, if we're working with spherical coordinates.
There we might want to use something like Haversine distance, which measure the Great Circle distance between two points.

For now we'll stick to Euclidean distance for simplicity.
Note that we specify `dims=1` so that the distances gives us the right shape (otherwise it wants to treat each column as an observation and each row as a feature, which is backwards)
"""

# ╔═╡ e29e2a84-1910-4a14-9ec5-feb824a0bc18
calc_dist(x1, x2) = Distances.pairwise(Distances.Euclidean(), x1, x2; dims=1);

# ╔═╡ 5fab821a-51c8-46d8-b1ae-992455c88260
md"Our data is 2D so we'll represnt it as a matrix."

# ╔═╡ 5200929f-853d-4cae-86a6-a4a23c3dcbaf
points = hcat(x, y);

# ╔═╡ 56cabc8f-b999-4662-a420-f6e71891384a
...
The cool thing is that nothing is different from the 1D case!
Now that we have our pairwise distances, we can define our kernel
"""

# ╔═╡ ce8fe84f-e70b-4173-897d-e85389f5b6a6
function sqexp_cov_fn(D, log_ℓ, log_σ)
    return exp(2 * log_σ) * exp.(-D .^ 2 ./ (2 * exp(2 * log_ℓ)))
end;

# ╔═╡ ba48ed3b-3131-404e-a3da-34fd975fd6f0
md"""
And we can write down a function to optimize our parameters as before
"""

# ╔═╡ 87f6f009-d1ab-48f4-acc4-c38089fdf5fb
function gp_logpdf(D, logσ, logℓ, logσy)
    N = size(D, 1)

    μ = ones(N) * mean(y)
    K = sqexp_cov_fn(D, logℓ, logσ)
    Σ = K + LinearAlgebra.I * exp(2 * logσy)

    joint_dist = MvNormal(μ, Σ)
    return logpdf(joint_dist, y)
end;
