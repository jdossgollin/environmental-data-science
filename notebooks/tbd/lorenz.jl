...
# ╔═╡ 3b335294-a8bd-437b-8f28-3a08767b4ae2
begin
...
# ╔═╡ a54a1c26-80c6-4603-96ad-22825a67837d
md"""
## Lorenz Attractor

I draw from [this discussion](https://mathworld.wolfram.com/LorenzAttractor.html), [this post](https://tng-daryl.medium.com/visualizing-the-lorenz-attractor-in-julia-c9417f219e9b), and [this tutorial](https://turing.ml/stable/tutorials/10-bayesian-differential-equations/).
...
# ╔═╡ 4da63a72-8609-11ec-1107-2d14fe5073b6
...
# ╔═╡ 43f82d46-013d-49c1-9f70-1d2aa1226ef7
md"Initial conditions"

# ╔═╡ c2893256-2018-4476-ad4f-b13cf52fd3b5
u0 = [1.0, 0.0, 0.0];

# ╔═╡ b0fc6577-1166-426d-b837-5d651b69600d
md"Timespan of the simulation. 100 in this case"

# ╔═╡ 33bfc369-1204-42e8-81a2-17e3c90081b1
tspan = (0.0, 30.0);

# ╔═╡ 5f749e6c-8fcb-4633-9cd0-ac74e7f604fc
md"Coefficients of the functions."

# ╔═╡ 140c6bd5-5e46-4484-b68c-d4993908792a
p = let
	σ = 10.0
	ρ = 28.0
	β = 8. / 3
...
# ╔═╡ f3d2be1a-7ad9-408a-8551-660918467e56
md"in the `DifferentialEquations.jl` notation we have to define the problem"

# ╔═╡ f7033bb4-bde5-4a3a-8afd-0ad713d8f77b
prob = ODEProblem(lorenz, u0, tspan, p);

# ╔═╡ 842ad359-f688-47df-bf0e-94a8687f1a74
solver = Tsit5();

# ╔═╡ 0bf45216-211a-4621-9440-bac67c0baf10
md"now we can solve"

# ╔═╡ 6b4f1912-0795-4086-a0f2-0ea89dd78531
sol = solve(prob, solver)

# ╔═╡ 80119c69-2ade-4abc-861b-70e4bf4c4351
md"We can plot this solution"

# ╔═╡ 8fc0faa3-8eba-4cd7-be3f-ded27b7837eb
plot1 = plot(
	sol, 
	vars = (1,2,3), 
	xlabel="x",  
...
# ╔═╡ 4756456d-3013-4c39-bceb-df47fb1dbe33
plot2 = plot(sol, label = ["x" "y" "z"])

# ╔═╡ e1070412-c009-4e67-ad3c-a16694f7bca4
md"We can get some data"

# ╔═╡ 5e372af6-7ac8-4265-b794-666b759398b3
Δt = 0.1; # how often are we saving data?

# ╔═╡ 1b8bae3f-2748-429a-8a11-5f5c3b22f11f
sol1 = solve(prob, solver, saveat=Δt)

# ╔═╡ c0f52103-c78f-44a5-a9a3-404892d2ebec
odedata = Array(sol1) + 0.8 * randn(size(Array(sol1)));

# ╔═╡ 6e51eb85-6374-4e63-81e3-b299ab8d4162
scatter!(plot2, sol1.t, odedata', label=false)

# ╔═╡ 38a259f2-bc99-43f7-94f0-4649a1ad50c9
md"""
## Fitting
"""

# ╔═╡ f5a523fc-b55d-4d2d-b640-04d832ec80eb
Turing.setadbackend(:forwarddiff)

# ╔═╡ 2e5b69c0-48c8-4056-b942-6e0e1aa7b621


# ╔═╡ 2111d227-cd35-4ce3-9bf8-ef0a29a1b90a
@model function fitlorenz(data, prob)
	...
	τ ~ LogNormal(0, 1) # the variance

	# this is the key piece here
    prob = remake(prob, p=p)
    predicted = solve(prob, solver, saveat=Δt)

    for i = 1:length(predicted)
        data[:,i] ~ MvNormal(predicted[i], τ)
    end
end;

# ╔═╡ 8a4b851f-461e-41f8-9843-b433a38287f7
model = fitlorenz(odedata, prob);

# ╔═╡ b8871946-ce87-4631-b41d-4f71b2976037
mle_estimate = optimize(model, MLE())

# ╔═╡ b3e7fcbb-2da1-438b-9726-105110ce80fe
p

# ╔═╡ 7b5443d3-0c79-4ca7-8b8b-bb382a49333f
map_estimate = optimize(model, MAP())

# ╔═╡ 1275ffbb-ca6f-4b68-a736-051e4b2b32e1
#chain = mapreduce(sample(model, DynamicNUTS(), 1000))

# ╔═╡ 13653b19-748c-4347-b4d6-d17c2361c221
plot(chain)

# ╔═╡ 87f3d85f-26be-4e71-a31c-a9b89476cf1d
let
	pl = scatter(sol1.t, odedata')
	chain_array = Array(chain)
	for k in 1:300
	    resol = solve(r
		emake(prob, p=chain_array[rand(1:1500), 1:4]),Tsit5(),saveat=0.1)
...
# ╔═╡ b2fa359d-04d0-4679-a7cc-972ca4eacf14
