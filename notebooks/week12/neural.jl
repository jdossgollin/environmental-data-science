...
Neural Networks are becoming increasingly popular tools for machine learning.
Although we'll explore small toy problems here, they work best when complicated models (lots of parameters) can be trained on very large data sets.

In class we discussed how many problems in machine learning can be decomposed into a couple of steps:

1. Write down a parametric model $y = f(x | \theta)$ where $x$ is data and $\theta$ are model parameters. This model includes linear regression, random forests, and much more
1. Define a loss function that tells us how good some values of $\theta$ are, using observed values of $x$ and $y$ to compare $y$ and $f(x | \theta)$
1. Derive a clever way to update the parameters $\theta$ given this loss function (*a very naive approach but easy to implement approach would be to randomly guess $\theta_\mathrm{new}$ and accept the proposal if it decreases the loss function*)
1. Keep iteratively improving the parameters $\theta$ until some stopping criterion is reached.

(This is an excessively general description of a large family of machine learning algorithms but provides a starting point for our conversation)

How does this framework apply to neural networks?

1. The parametric model $y = f(x | \theta)$ is the neural network. To paint with a very broad brush again, neural networks combine addition, multiplication, and nonlinear "activation functions" in layers. There are theorems proving that in theory neural networks can approximate any well-behaved function (though in practice with finite training you may not get there!)
1. Lots of loss functions can be used
1. Neural networks use back-propagation. That is, the gradient of the loss function with respect to parameters, given a particular $x$, can be computed easily. This is used to step the parameters along the gradient. Neural networks work well because accurate gradients can be computed (at low computational cost) even for very complex models -- thus they scale well.
"""

# ╔═╡ 72901e31-bead-4496-8822-206aad162f31
md"""
## Recommended Resources

* Video: [What is backpropagation really doing?](https://www.youtube.com/watch?v=Ilg3gGewQ5U&feature=youtu.be) -- builds intuition about what backprop is doing
* Blog: [A Step by Step Backpropagation Example](https://mattmazur.com/2015/03/17/a-step-by-step-backpropagation-example/) -- gives a step by step example of backpropoagation
* Iconic Video: [Livecoding Madness - Let's Build a Deep Learning Library](https://www.youtube.com/watch?v=o64FV-ez6Gw) -- Joel Grus builds a (simple) neural network library from scratch in an hour. Great way to learn Python and to see whether you really understand deep learning
* Book: [Deep Learning Book](https://www.deeplearningbook.org/) by Ian Goodfellow and Yoshua Bengio and Aaron Courville (free to read) -- great and detailed reference
"""

# ╔═╡ aff13e86-8182-498f-ad5c-8b922d320168
md"""
## Gradients

The most important thing that a neural network library can do is take gradients.
There are clever auto-diff libraries that can calculate gradients using a combination of analytic and numerical methods.

The `Flux.jl` package implements a full neural network library, but we'll only use its autodiff tools. 
Specifically, Flux exports the `gradient` function.
Let's define a simple 1D function and its derivatives
```math
\begin{align}
f(x) &= 3 x^2 + 2x + 1 \\
\frac{df}{dx} &= 6x + 2 \\
...
\end{align}
```
We can also define a naive autodiff function that uses the relation
```math
\frac{df}{dx} |_{x'} = \lim_{h \rightarrow 0} \frac{f(x' + h) - f(x' - h)}{2h}
```
to approximate the derivative by plugging in a small value of $h$ (this is a very naive and computationally expensive version of how `gradient()` works)
"""
...
md"""
We see that we're basically getting the right answer, but that our cheap numerical method is a bit noisy while the smarter implementations don't need this.
Here's another example of a much more complicated function
"""
...
md"""
Depending on what we define $g(x)$ to be, we can get pretty good results

The gradient function also works if we have multidimensional inputs:
"""
...
md"""
this is the gradient with respect to each element in $x$ and $y$
"""

# ╔═╡ 9a0f0b94-8b87-4875-9a4d-11f9e5518b12
md"""
...
1. Calculate the gradient of the loss function with respect to the parameters of the model, holding the data constant
1. Use this gradient to step each parameter along the gradient
"""

# ╔═╡ c03c575f-6191-4f1f-b315-fb75626b775b
md"""
Here's how we use this to make predictions.
"""
...
md"""
We don't know these parameters so let's guess
"""
...
# ╔═╡ fae20608-8e16-4b37-9c7b-2aa171cc7365
md"""
We can use these guesses to make predictions
"""

# ╔═╡ 97953b18-9cc3-4610-92a5-0a4a25cd4865
lin_reg(X_reg, α_guess, β_guess)

# ╔═╡ e9d5b539-6f90-4bf7-9cc2-3943f23be9c9
md"""
Of course these predictions may not be very good -- we're just randomly guessing what the slopes and intercept are.
To improve, let's define a loss function -- mean squared error is a good guess.
"""
...
# ╔═╡ 48f7f007-53e6-4021-b212-32c99c408f27
md"""
We can use this to calculate the gradient of the loss function with respect to our parameters
"""
...
grad[α_guess], grad[β_guess]

# ╔═╡ 019babff-97ae-4f40-9157-bde6ad317b37
md"""
If we step along this gradient, we can improve our predictions
"""
...
md"""
We can even train until we get stable predictions
"""
...
Now that we've gotten those steps out of the way we can start to build a (simple) neural network from scratch.
"""
...
For our purposes, let's just define a linear layer.

During the forward pass, the linear layer takes 

1. an input $X$ of shape $N \times D$
1. a weight matrix $W$ of shape $D \times M$, and
1. a bias matrix $B$ of shape $N \times M$

and computes an output $Y = XW + b$ of shape $N \times M$.

A linear layer can also have an "activation function" $\sigma$ so that it outputs $\sigma(Y)$
"""
...
md"""
Typically neural networks have multiple layers.
Let's simulate 100 obervations of a variable $X$ with 15 dimensions.
Then we will use 3 layers to make predictions about some y
(since the parameters are all guesses, these predictions won't be meaningful!)
"""

# ╔═╡ ddb7df67-734b-45ed-aabb-b219acf7e908
relu(x) = max(x, 0);
...
# ╔═╡ 8716720a-c507-4bda-b7da-03c61bcd4e48
md"""
We can use this to compute the loss function
"""

# ╔═╡ 402dc527-fb88-4c2d-9cf8-581adc235900
loss(x, y) = MSE(model(x), y);

# ╔═╡ e396f3e4-ec61-4291-912f-70c2666d065f
loss(X_nn, y_nn)

# ╔═╡ 7a2efef5-ce03-469d-879b-3ff4747ff95f
md"""
We can also compute the gradient of the loss function with respect to each of the parameters!
"""

# ╔═╡ 9c0fcb5e-5b6f-4e7b-8f4a-950976cbfeec
...
md"""
There is a gradient for each parameter (the interface is a little tricky because it's not really intended to be used by hand).
There are six parameters: W and b for each of 4 layers
"""

# ╔═╡ 6dacc4ca-d34f-43c3-b98d-9018c15a5feb
length(grad_nn.params)

# ╔═╡ bc6ed3a1-c597-400d-a853-23c001c7dc48
md"""
We can access each of them
"""
...
We're not going to cover all of neural networks in a short notebook.
If you're interested in learning more there are classes and textbooks!
But take a few key points with you

1. Neural networks work by applying the chain rule on layers that function like $h(g(f(x)))$. They're not magic!
