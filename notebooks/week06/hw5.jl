...
```math
\begin{align}
\log\alpha_i &= b_\alpha X_i \\
\log\beta_i &= b_\beta X_i
\end{align}
```
but this turns out not to be very practical.
...
function simulate_zero_inflated_beta(; β, ϕ, N)
	X = make_X(N)
	y = 1
	return y
end;
...
begin
	β = [1, 1] # FILL IN -- MAKE SURE IT'S RIGHT DIMENSIONS
	ϕ = 1 # FILL IN
	N = 100
	y = simulate_zero_inflated_beta(β=β, ϕ=ϕ, N=N)
end;
...
md"""
From the Gelman golf putting example, fit the angle-based model to the "new data" and plot your results.
